...
| Native global-state locking  | eSpeak public API global behavior                       | Delegate process-wide locking and lifetime to `espeakng-runtime`.                                    |
| Raw blocks                   | Observed eSpeak/Piper behavior                          | Use a small state-machine parser and independent composition helper.                                 |
| CLI fallback                 | Existing project behavior                               | Delegate CLI execution to `espeakng-runtime` while retaining Piper's local splitter and composition. |
