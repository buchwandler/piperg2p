...
def _compare_cases(
    ...
    for case in actual:
        ...
        expected_value = {
            key: reference.get(key) for key in ("phoneme_type", "sentences")
        }
        comparisons.append(
            {
                "id": case["id"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--corpus", type=Path, default=Path(__file__).parent / "data" / "core.json"
    )
    parser.add_argument(
        "--config",
        type=Path,
        help="Voice config used to execute compatible corpus cases",
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Direct .g2lex asset for overlay mode",
    )
    parser.add_argument(
        "--expected", type=Path, help="Reviewed expected output to compare against"
    )
    parser.add_argument(
        "--write-expected",
        action="store_true",
        help="Explicitly replace the expected output",
    )
    ...
    if args.config is None:
        ...
        lookup = (
            G2LexLookup(args.lexicon, language=config.espeak_voice)
            if args.mode == "overlay"
            else None
        )
        try:
            frontend = (
                PiperFrontend(config, lexicon_backend=lookup)
                if lookup is not None
                else PiperFrontend(config)
            )
            try:
    ...
    return (
        1
        if report.get("comparison", {}).get("status")
        in {"mismatch", "missing-expected"}
        else 0
    )
